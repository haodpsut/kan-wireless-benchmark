...
\author{Phuc Hao Do%
\thanks{P. H. Do is with Da Nang Architecture University, Da Nang, Vietnam
...
These timings are single-thread CPU measurements; on a massively parallel accelerator the
wall-clock gap would narrow, since the extra per-edge operations parallelize. The
\emph{arithmetic}, however, does not: the multiply-add and transcendental counts
\eqref{eq:cmlp}--\eqref{eq:ckans} are device-independent, so the energy and throughput penalty
remains even where latency is hidden. We report CPU latency as the transparent proxy and the
FLOP counts as the device-independent statement.
