...
trained over $5$ seeds. We evaluate \emph{behavioral modeling}, the forward-model accuracy
...
in the linear-Gaussian setting, and is the genuine classical workhorse here; we also include
simple linear interpolation as a weak reference.
...
is the strongest matched-budget model, so the KAN is not beaten by an untuned baseline.}
\label{tab:mlphp}
\begin{tabular}{lrr}
\toprule
...
does substantially more work. An MLP layer costs $O(d_{\mathrm{in}}d_{\mathrm{out}})$
multiply-adds, a Fourier-KAN layer costs $O(d_{\mathrm{in}}d_{\mathrm{out}}K)$ with an extra
$2K$ transcendental evaluations per edge, and a spline-KAN layer costs
$O(d_{\mathrm{in}}d_{\mathrm{out}}(G+k))$ with a B-spline recurrence per edge. The parameter
count absorbs the $K$ or $G{+}k$ factor into the weight tensor, but the arithmetic does not
disappear; it reappears as latency.
