...
\begin{document}
\title{When Does a Kolmogorov--Arnold Network Help in Wireless?\\
A Parameter-Matched, Non-Saturated Benchmark}

\author{\IEEEauthorblockN{Do Phuc Hao}
\IEEEauthorblockA{Da Nang Architecture University, Da Nang, Vietnam\\
Email: haodp@dau.edu.vn}%
\thanks{Manuscript prepared August 2026. Code and data-preparation scripts are
released to reproduce every reported number (Section~\ref{sec:repro}).}}

\markboth{Preprint, August 2026}%
{Do: When Does a Kolmogorov--Arnold Network Help in Wireless?}
...
on two non-saturated wireless regimes: power-amplifier behavioral modeling (a smooth
low-dimensional regression) and automatic modulation classification on the real RML2016.10a
dataset. The honest picture is regime-dependent. KANs are genuinely competitive and
...
strawmen.
\item We evaluate two wireless regimes chosen to be \textbf{non-saturated}, so
differences are resolvable: (i) power-amplifier behavioral modeling / DPD, a smooth
low-dimensional regression that is the regime most favorable to KANs
(Section~\ref{sec:dpd}); and (ii) automatic modulation classification (AMC) on the real
RML2016.10a dataset restricted to the low-SNR band (Section~\ref{sec:amc}).
...
\item We run \textbf{four ablations} (Section~\ref{sec:abl}) that stress-test the comparison
in KAN's favor: a parameter-budget sweep, a KAN hyperparameter sweep, a training-set-size
sweep, and an SNR sweep. These reveal honest crossovers, KANs win in some regimes, and pin
down where.
...
every model to it. The Fourier-KAN sets $P$; the spline-KAN hidden width, the MLP hidden
width, and (where used) are then chosen to land within a few percent of $P$. This makes
``KAN uses fewer parameters'' testable rather than assumed. For DPD all learned models
sit at $P\approx\input{claims/dpd_params}$ parameters.
...
several seeds; we report the mean with a bootstrap $95\%$ confidence interval, and for the
head-to-head question we report the \emph{paired} KAN-minus-baseline difference with its
own interval. No result rests on a single seed.
...
capture a one-dimensional AM/AM-like curve. We drive a Wiener--Hammerstein PA, an input
FIR filter followed by a static Saleh AM/AM and AM/PM nonlinearity~\cite{saleh1981} and an
output FIR filter, with a pulse-shaped 16-QAM baseband signal at $6$~dB input backoff.
Because the ground-truth PA is \emph{not} a memory polynomial, the classical
memory-polynomial baseline gets no unfair ``true model class'' advantage; all learners
approximate. The task is to predict the complex PA output from a length-$5$ memory window
of the complex input; the metric is normalized mean-squared error (NMSE) in dB. Models are
trained over $5$ seeds.
...
is not a strawman: a properly built generalized memory polynomial (GMP), with the
lagging/leading cross-envelope terms of the standard model~\cite{morgan2006gmp}, reaches
$\input{claims/dpd_mempoly_nmse}$~dB, statistically tied with the best neural model and better
than both KANs. So at the \emph{default} matched configuration, even on the regime most
...
3 seeds. Accuracy (higher is better).}
\label{tab:amc}
\begin{tabular}{lrr}
\toprule
Model & Params & Accuracy $\uparrow$ \\
\midrule
CNN (task-appropriate) & $5291$  & $\mathbf{0.660\pm0.004}$ \\
MLP (matched)          & $68651$ & $0.495\pm0.002$ \\
KAN, spline (canon.)   & $69420$ & $0.466\pm0.002$ \\
KAN, Fourier           & $68395$ & $0.354\pm0.007$ \\
\bottomrule
\end{tabular}
\end{table}
...
Resolving accuracy by modulation (Fig.~\ref{fig:permod}) sharpens the picture and connects it
to signal structure. The spline-KAN's advantage over the matched MLP is not spread evenly: it
concentrates on the phase- and amplitude-shift modulations whose decision regions are
constellation geometry, gaining $+\input{claims/permod_kan_bpsk_gain}$ on BPSK,
$+\input{claims/permod_kan_pam4_gain}$ on PAM4, and $+\input{claims/permod_kan_8psk_gain}$ on
8PSK, while it slightly trails the MLP on the analog and frequency-modulated classes (for
example $\input{claims/permod_kan_gfsk_loss}$ on GFSK). This is exactly the pattern the
per-edge mechanism predicts: a learnable univariate function per input can carve the smooth
amplitude/phase boundaries of a constellation, but offers little for signals whose class is
carried by frequency dynamics that a flat model reads only weakly in any case. The KAN is
therefore not a generically better classifier; it is a better fit to a specific, identifiable
structure.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_permod.pdf}
\caption{Per-modulation accuracy difference, spline-KAN minus matched MLP. The KAN advantage
concentrates on constellation-geometry modulations (BPSK, PAM4, 8PSK, QPSK) and reverses on
frequency/analog modulations, consistent with a per-edge decision-boundary mechanism.}
\label{fig:permod}
\end{figure}
...
cubic. The fits are excellent: the median per-edge $R^2$ is
$\input{claims/interp_median_r2}$ and $\input{claims/interp_frac_simple_pct}\%$ of edges
exceed $R^2=0.95$ (Fig.~\ref{fig:interp}). Fig.~\ref{fig:edges} makes this concrete by
...
confirming the edges are simple, readable curves.}
\label{fig:edges}
\end{figure}

\emph{(Q2) Fidelity: is it free?} We then \emph{distill} layer one into those cubics (a
symbolic surrogate), keep the rest of the network exact, and re-measure NMSE. The full KAN
scores $\input{claims/interp_nmse_full}$~dB; the symbolic surrogate scores
$\input{claims/interp_nmse_symbolic}$~dB, a degradation of
$\input{claims/interp_degradation_db}$~dB. In other words, the readable form is not the
performant form: extracting the interpretable symbols discards more than a decibel of
modeling accuracy. Interpretability here is real but not free, which is precisely the
...
with median $R^2=0.998$), but distilling them into that symbolic form costs $1.27$~dB of
...
\emph{KAN hyperparameters (Table~\ref{tab:kanhp}).} We then tune the KAN itself. Increasing
the Fourier basis order to $K=8$ raises the Fourier-KAN to
$\input{claims/dpd_fkan_k8_nmse}$~dB at $\input{claims/dpd_fkan_k8_params}$ parameters,
which \emph{beats} a same-budget MLP ($\input{claims/dpd_mlp_h48_nmse}$~dB at $4658$).
Spectral richness, not width, is the lever, and with it the Fourier-KAN wins the regression
task at matched capacity. This is the flip side of the capacity-confound argument: just as an
unmatched KAN can win for the wrong reason, a well-chosen KAN can win for the right one, and
only a matched comparison tells them apart.
...
\begin{table}[t]
\centering
\caption{DPD KAN hyperparameter sweep (5 seeds). A richer Fourier basis ($K{=}8$) beats a
same-budget MLP ($-16.52$~dB at $4658$ params).}
\label{tab:kanhp}
\begin{tabular}{llrr}
\toprule
...
wins only at low SNR, while the spline-KAN overtakes it above $\sim 0$~dB. Reporting a single
...
We tested two regimes, two KAN variants, and one dataset per regime; a broader sweep (more
...
reader can regenerate the results and verify the paper's numbers against the CSVs.
